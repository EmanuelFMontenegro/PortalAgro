...
export class RecuperContrasenaComponent {
  ...
  constructor(private apiService: ApiService, private toastr: ToastrService, private formBuilder: FormBuilder, private router: Router) {
      ...
      this.apiService.recuperarContrasena(email).subscribe({
        next: (response) => {
          if (response.status === 200) {
            this.toastr.info('Correo con instrucciones de recuperación enviado con éxito. Revisa tu correo electrónico para continuar.');
